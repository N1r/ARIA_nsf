...
class LTVZeroPhaseFIRFilterPrecise(LTVFilterInterface):
    def __init__(self, window: str, n_mag: int = None, mag_activation: str = "exp"):
        super().__init__()
        self.window_fn = get_window_fn(window)
        # "exp": unbounded positive magnitude (default; noise spectral envelope).
        # "sigmoid": magnitude bounded to [0, 1] — turns the filter into a per-band
        #            mixing/aperiodicity weight so it can be used in a crossfade
        #            (I - H)·harmonic + H·noise without the subtraction going negative.
        self.mag_activation = mag_activation

        if n_mag is not None:
            self.ctrl = wrap_ctrl_fn(split_size=(n_mag,), trsfm_fn=lambda x: (x,))

    def _mag(self, log_mag: Tensor) -> Tensor:
        if self.mag_activation == "exp":
            return torch.exp(log_mag)
        elif self.mag_activation == "sigmoid":
            return torch.sigmoid(log_mag)
        raise ValueError(f"Unknown mag_activation: {self.mag_activation}")

    def get_zero_phase_fir(self, log_mag: Tensor):
        mag = self._mag(log_mag) + 0j
...
class LTVZeroPhaseFIRFilter(LTVZeroPhaseFIRFilterPrecise):
    def __init__(self, window: str, conv_method: str = "direct", n_mag: int = None,
                 mag_activation: str = "exp"):
        super().__init__(window=window, n_mag=n_mag, mag_activation=mag_activation)
        if conv_method == "direct":
            self.convolve_fn = F.conv1d
        elif conv_method == "fft":
            self.convolve_fn = fft_conv1d
...
class AperiodicityFIRFilter(LTVZeroPhaseFIRFilter):
    """Zero-phase FIR whose magnitude response is a bounded per-band aperiodicity
    A(f) = sigmoid(logits + freq_bias) in [0, 1], used as a frequency-dependent
    voiced/noise mixing weight in a crossfade
        src = (I - A)·harmonic + A·noise.

    `hf_bias` installs a fixed frequency-rising prior (−hf_bias at DC → +hf_bias at
    Nyquist) so that at init the low band is harmonic-dominated (A≈0) and the high
    band is noise-dominated (A≈1) — i.e. a physically-motivated HNR prior that
    dissolves the coherent high-frequency harmonic stripes. The encoder can still
    override it per-frame (e.g. push A→1 across the band for fricatives)."""

    def __init__(self, window: str, conv_method: str = "direct", n_mag: int = None,
                 hf_bias: float = 2.0):
        super().__init__(window, conv_method, n_mag, mag_activation="sigmoid")
        if n_mag is not None:
            bias = torch.linspace(-hf_bias, hf_bias, n_mag)
    ...
    def _mag(self, log_mag: Tensor) -> Tensor:
        if self.freq_bias is not None:
            log_mag = log_mag + self.freq_bias
        return torch.sigmoid(log_mag)
    ...
    for key, value in config.items():
        if key == "class_path":
            if ".LTVMinimumPhaseFilter" in config["class_path"]:
                config["class_path"] = "aris.models.filters.LTVMinimumPhaseFilterPrecise"
                return config
            elif ".LTVMinimumPhaseFIRFilter" in config["class_path"]:
                config["class_path"] = "aris.models.filters.LTVMinimumPhaseFIRFilterPrecise"
                config["init_args"].pop("conv_method")
                return config
            elif ".LTVZeroPhaseFIRFilter" in config["class_path"]:
                config["class_path"] = "aris.models.filters.LTVZeroPhaseFIRFilterPrecise"
                config["init_args"].pop("conv_method")
                return config
