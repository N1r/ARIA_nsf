...
def create_experiment(
    ...
    *,
    ...
    if sample_rates != {16000}:
        raise ValueError(
            "The bundled presets currently require 16 kHz audio; run phonlab prepare "
    ...
    (output / "train.sh").write_text(
        ...
        '"$PROJECT_ROOT/.venv/bin/python" -m phonlab_ddsp.cli train "$EXPERIMENT_DIR"\n',
        encoding="utf-8",
    )
...
def _training_config(manifest, run_dir, **settings) -> str:
    ...
    return f"""# Generated by PhonLab-DDSP. Edit a copy; experiment.json records its hash.
...
data:
  class_path: phonlab_ddsp.lightning.ManifestDataModule
  init_args:
