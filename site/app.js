...
const GLOSS = {
  ...
  f2: "<b>F2</b> — backness (front ↔ back): a bright front /i/ darkens toward a back /u/. F1 stays low (slight cascade coupling on higher-F1 vowels).",
...
function renderPraat() {
  ...
  fetch(ASSET + "praat_cont.json").then(r => r.ok ? r.json() : null).then(m => {
    ...
    buildPills("cmpUtt", m.utts.map((u, i) => ({ k: i, label: u.voice ? `${u.voice} · ${u.label}` : u.label })), cst.u, k => { cmpStop(); cst.u = +k; cmpRender(); });
    const sl = document.getElementById("cmpStep");
    sl.max = m.steps.length - 1; sl.value = cst.step;
    sl.oninput = e => { cmpStop(); cst.step = +e.target.value; cmpRender(); };
    host.addEventListener("click", e => {
      const b = e.target.closest(".cmp-sweep"); if (b) cmpSweep(b.dataset.u, b.dataset.p, b.dataset.s);
    });
    cmpRender();
  }).catch(() => { host.innerHTML = '<p class="muted">comparison audio unavailable.</p>'; });
}
function cmpSrc(uid, pid, st, sys, ext) { return `${ASSET}cont/${uid}__${pid}_${st}__${sys}.${ext}`; }
function cmpRender() {
  if (!CONT) return;
...
function cmpSweep(uid, pid, sys) {
  cmpStop();
  const steps = CONT.steps; let i = 0;
  const a = new Audio(); cmpAudio = a;
  const next = () => {
    if (a !== cmpAudio || i >= steps.length) return;
    cst.step = i;
    const sl = document.getElementById("cmpStep"); if (sl) sl.value = i;
    cmpRender();                                   // specs + step label follow the sweep
    a.src = cmpSrc(uid, pid, steps[i], sys, "wav"); i++; a.play().catch(() => {});
  };
  a.onended = next; next();
}
...
function drawCompanion() {
  ...
  if (c.ltas) {
    ...
    cap.innerHTML = "Long-term spectrum, <b>oral → nasal</b>: the F1 peak amplitude <b>A1 drops</b> and " +
      ...
      "<i>frequencies</i> are held; the dotted line marks the ~250 Hz P0 nasal-pole region (reference — " +
    ...
    cap.innerHTML = "Formant-pole trajectory placed by the model for the three syllables (the synthesiser's poles, not Praat-measured — acoustic re-measurement is in the Fidelity section); the current step is ringed. " + (
